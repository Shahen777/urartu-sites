/* БРЫНЗА · Анапа — интерактив: курсор, reveal, parallax, drag, счётчики, отзывы, лайтбокс */
(() => {
  ...
  /* --- parallax картинок --- */
  if (!mq) {
    const phs = [...document.querySelectorAll('.split__ph img, .mosaic figure img')];
    let ticking = false;
    const par = () => {
      phs.forEach(img => {
        const r = img.closest('figure, .split__ph').getBoundingClientRect();
  ...
  const cio = new IntersectionObserver(es => es.forEach(e => {
    ...
    const step = t => {
      ...
      b.textContent = (b.dataset.prefix || '') + v.toFixed(dec) + (b.dataset.suffix || '');
