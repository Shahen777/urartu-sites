/* МАЦОНИ — интерактив: курсор, reveal, parallax, drag, счётчики, отзывы, fab, лайтбокс */
(() => {
  ...
  /* --- parallax картинок --- */
  if (!mq) {
    const phs = [...document.querySelectorAll('.split__ph img, .mosaic figure img, .kids__media figure img')];
    let ticking = false;
    const par = () => {
      phs.forEach(img => {
