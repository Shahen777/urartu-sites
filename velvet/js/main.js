/* VELVET — интерактив: курсор, reveal, parallax, drag, счётчики, отзывы, лайтбокс */
(() => {
  ...
  /* --- parallax картинок --- */
  if (!mq) {
    const phs = [...document.querySelectorAll('.split__ph img, .mosaic figure img, .awards__ph img')];
    let ticking = false;
    const par = () => {
      phs.forEach(img => {
        const r = img.closest('figure, .split__ph, .awards__ph').getBoundingClientRect();
